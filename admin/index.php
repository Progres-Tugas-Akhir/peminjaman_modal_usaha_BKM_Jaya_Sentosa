...
if(isset($_POST['login'])){
	...
	$login="SELECT * FROM tb_pengurus_lembaga WHERE username='$username' AND password='$password'";
	...
	if ($ketemu > 0){
		...
		<script type="text/javascript">
			alert("Anda Login Sebagai Pengurus Lembaga");
		</script>
		<?php
		echo "<script>document.location='h_user/index.php'</script>";
	}else{
		if($username=="kepala" && $password==md5("kutorejo")){
			session_start();
			$_SESSION['nama'] 		= "Kepala";
			?>
		<script type="text/javascript">
			alert("Anda Login Sebagai Kepala Kelurahan");
		</script>
		<?php
			echo "<script>document.location='h_kepala/index.php'</script>";
	}else{
		if($username=="admin" && $password==md5("jayasentosa")){
			session_start();
			$_SESSION['nama'] 		= "Admin";
			?>
		<script type="text/javascript">
											...
											<form method="post" action="">
												<fieldset>
													...
													<label class="block clearfix">
														...
														<button class="width-35 pull-right btn btn-sm btn-warning" href="register.php">Register</button>
													</button>
